...
CORE_2006="""
NUMBER FIELD NAME POSITION LENGTH FIELD TYPE
...
64 TOTAL_NON_COV_CHARGES_ACCOMM 514 12 Numeric
65 TOTAL_CHARGES_ANCIL 526 12 Numeric
66 TOTAL_NON_COV_CHARGES_ANCIL 538 12 Numeric
...
246 HCFA_MDC 1442 2 Alphanumeric
247 APR_MDC 1444 2 Alphanumeric
248 HCFA_DRG 1446 3 Alphanumeric
...
for y,line in enumerate(TABBED_FORMATS.split("\n")):
    if line.strip():
        for i,k in enumerate(line.strip().split('\t')):
            fields[k][y] = i
for f,v in fields.iteritems():
    temp = {k for k in v.values()} # print f,temp,len(v)
    if len(temp) > 1 or len(v) != 16:
        print "field {} is inconsistent".format(f)
        print f,v,temp
        raise ValueError
    else:
        cleaned_fields[f] = temp.pop()
# for k,v in cleaned_fields.iteritems():
#     print k,v


def get_element(name,line):
    return line[cleaned_fields[name]].strip()

ELEMENTS = {}
for k in CORE_2006.splitlines():
    if k.strip() and k[0].isdigit():
        try:
            k = k.replace(' (fixed length format only)','').split(' ')
            ELEMENTS[k[1]] = (int(k[2])-1,int(k[2])-1+int(k[3]))
        except:
            raise ValueError,k
if CORE_2008 != CORE_2007 or CORE_2007 != CORE_2006 or CORE_2006 != CORE_2008:
    raise ValueError

def get_element_fixed(name,line):
    return line[ELEMENTS[name][0]:ELEMENTS[name][1]].strip()
